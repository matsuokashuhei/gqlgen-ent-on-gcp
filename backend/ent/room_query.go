...
func (rq *RoomQuery) QueryStudio() *StudioQuery {
	query := &StudioQuery{config: rq.config}
...
func (rq *RoomQuery) QuerySchedules() *ScheduleQuery {
	query := &ScheduleQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) WithStudio(opts ...func(*StudioQuery)) *RoomQuery {
	query := &StudioQuery{config: rq.config}
...
func (rq *RoomQuery) WithSchedules(opts ...func(*ScheduleQuery)) *RoomQuery {
	query := &ScheduleQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withSchedules = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	var (
		...
		loadedTypes = [2]bool{
			rq.withStudio != nil,
			rq.withSchedules != nil,
		}
	)
	if rq.withStudio != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withStudio; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if nodes[i].studio_rooms == nil {
				continue
			}
			fk := *nodes[i].studio_rooms
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "studio_rooms" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Studio = n
			}
		}
	}

	if query := rq.withSchedules; query != nil {
		...
		query.Where(predicate.Schedule(func(s *sql.Selector) {
			s.Where(sql.InValues(room.SchedulesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.room_schedules
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_schedules" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_schedules" returned %v for node %v`, *fk, n.ID)
			}

...
func (cq *ClassQuery) QueryInstructor() *InstructorQuery {
	query := &InstructorQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ClassQuery) WithInstructor(opts ...func(*InstructorQuery)) *ClassQuery {
	query := &InstructorQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ClassQuery) sqlAll(ctx context.Context) ([]*Class, error) {
	...
	if query := cq.withSchedule; query != nil {
		...
		for i := range nodes {
			if nodes[i].schedule_classes == nil {
				continue
			}
			fk := *nodes[i].schedule_classes
	...
	if query := cq.withInstructor; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Class)
		for i := range nodes {
			if nodes[i].instructor_classes == nil {
				continue
			}
			fk := *nodes[i].instructor_classes
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(instructor.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "instructor_classes" returned %v`, n.ID)
			}
			for i := range nodes {

...
func (sc *SchoolCreate) AddStudioIDs(ids ...int) *SchoolCreate {
	sc.mutation.AddStudioIDs(ids...)
	return sc
}

// AddStudios adds the "studios" edges to the Studio entity.
func (sc *SchoolCreate) AddStudios(s ...*Studio) *SchoolCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return sc.AddStudioIDs(ids...)
}
...
func (sc *SchoolCreate) check() error {
	if _, ok := sc.mutation.CreateTime(); !ok {
		return &ValidationError{Name: "create_time", err: errors.New(`ent: missing required field "School.create_time"`)}
	}
	if _, ok := sc.mutation.UpdateTime(); !ok {
		return &ValidationError{Name: "update_time", err: errors.New(`ent: missing required field "School.update_time"`)}
	}
...
func (sc *SchoolCreate) createSpec() (*School, *sqlgraph.CreateSpec) {
	...
	if nodes := sc.mutation.StudiosIDs(); len(nodes) > 0 {

...
func (sq *SchoolQuery) QueryStudios() *StudioQuery {
	query := &StudioQuery{config: sq.config}
...
func (sq *SchoolQuery) WithStudios(opts ...func(*StudioQuery)) *SchoolQuery {
	query := &StudioQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withStudios = query
	return sq
}
...
func (sq *SchoolQuery) sqlAll(ctx context.Context) ([]*School, error) {
	...
	if query := sq.withStudios; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*School)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Studios = []*Studio{}
		}
		query.withFKs = true
		query.Where(predicate.Studio(func(s *sql.Selector) {
			s.Where(sql.InValues(school.StudiosColumn, fks...))
		}))

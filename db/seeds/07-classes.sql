...
INSERT INTO classes (name, tuition, start_date, instructor_classes, schedule_classes) VALUE ("Jr.HIPHOP 小学生", 6000, "2022-01-01", @instructor_id, @schedule_id);
SELECT id INTO @schedule_id FROM schedules WHERE room_schedules = @room_id AND day_of_week = 1 AND start_time = "18:00";
INSERT INTO classes (name, tuition, start_date, instructor_classes, schedule_classes) VALUE ("GirlsHIPHOP 初級", 7000, "2022-01-01", @instructor_id, @schedule_id);
SELECT id INTO @schedule_id FROM schedules WHERE room_schedules = @room_id AND day_of_week = 1 AND start_time = "19:15";
INSERT INTO classes (name, tuition, start_date, instructor_classes, schedule_classes) VALUE ("GirlsHIPHOP 中級", 6000, "2022-01-01", @instructor_id, @schedule_id);
